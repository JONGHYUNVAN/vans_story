...
export default function PostCard<T extends BasePost>({ 
  ...
  return (
    <article 
      className="group relative rounded-lg bg-[#0d1117] text-white hover:shadow-lg hover:shadow-[#2496ED]/10 p-6 border border-slate-700/40"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* 배경 레이어 */}
      <div className="absolute inset-0 bg-gradient-to-b from-[#121920] to-[#0a1017] z-0 rounded-lg"></div>
      
      {/* 그라데이션 배경 효과로 대체 */}
      <div className="absolute inset-0 bg-gradient-to-r from-[#2496ED]/5 to-transparent opacity-0 group-hover:opacity-20 transition-opacity duration-300 rounded-lg z-0" />
      
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-lg font-medium text-white group-hover:text-[#2496ED]/80 transition-colors duration-300">
            {post.title}
          </h2>
        ...
        {post.topic && (
          <p className="text-xs text-[#2496ED]/60 -mt-1">{post.topic}</p>
        )}
        
        {/* 가로 구분선 - 중앙에서 확장 */}
        <div className="relative h-[1px] my-2 overflow-hidden">
          <div 
            className="absolute inset-0 mx-auto bg-gradient-to-r from-transparent via-[#2496ED]/30 to-transparent"
            style={{
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="relative w-24 h-24 rounded-md overflow-hidden flex-shrink-0 border border-slate-700/60">
                    <img
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = '/docker.webp';
                      }}
                    />
                  </div>
        ...
        <div className="flex items-center justify-between text-xs text-gray-300">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1">
              <AiOutlineEye className="w-4 h-4 text-[#2496ED]/60" />
              {post.viewCount}
            </span>
            <span className="flex items-center gap-1">
              <AiOutlineLike className="w-4 h-4 text-[#2496ED]/60" />
              {post.likeCount}
            </span>
          </div>
