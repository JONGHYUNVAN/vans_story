...
export default function PostCard<T extends BasePost>({ 
  ...
  return (
    <article 
      className="group relative rounded-lg bg-white text-black hover:shadow-lg hover:shadow-green-400/20 p-6"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-lg font-medium text-gray-800 group-hover:text-black transition-colors">
          {post.title}
        </h2>
        <div className="flex items-center gap-3">
          {post.author && (
            <span className="text-xs text-black">{t('post.by', { author: post.author })}</span>
          )}
      ...
      {post.topic && (
        <p className="text-xs text-black -mt-1">{post.topic}</p>
      )}
      
      {/* 가로 구분선 - 중앙에서 확장 */}
      <div className="relative h-[1px] my-2 overflow-hidden">
        <div 
          className="absolute inset-0 mx-auto bg-gradient-to-r from-transparent via-green-500 to-transparent"
          style={{
      ...
      >
        ...
        >
          ...
          {(isHovered || showDescription) && (
            <div className="text-black text-sm mb-4 w-full">
