...
export default function PostCard({ 
      ...
      <div>
        {lines.map((line, index) => (
          <div key={index} style={{ marginBottom: index < lines.length - 1 ? '0.5rem' : 0 }}>
            ...
            {index === lastLineIndex && (
              <span
                className="inline-block border-r-2 border-gray-800 h-4 ml-[1px]"
                style={{ 
  ...
  return (
    <article 
      className="group relative rounded-lg bg-white text-gray-800 hover:shadow-lg hover:shadow-gray-200/30 p-6 border border-black"
    ...
    >
      ...
      <div className="relative h-[1px] my-2 overflow-hidden">
        <div 
          className="absolute inset-0 mx-auto bg-gradient-to-r from-transparent via-black to-transparent"
          style={{
          ...
          {(isHovered || showDescription) && (
            <div className="text-gray-700 text-sm mb-4 w-full">
              <div className="flex items-start gap-4">
                {/* 썸네일 이미지 */}
                <div className="relative w-24 h-24 rounded-md overflow-hidden flex-shrink-0 border border-black/30">
                  <img
                    src={post.thumbnail || '/algorithm.webp'}
                    alt="thumbnail"
                    className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                    onError={(e) => {
                      // 이미지 로드 실패 시 기본 이미지로 대체
                      (e.target as HTMLImageElement).src = '/algorithm.webp';
                    }}
                  />
                </div>
