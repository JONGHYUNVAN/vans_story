...
export default function PostCard<T extends BasePost>({ 
  ...
  return (
    <article 
      className="group relative rounded-lg bg-[#0a0a0a] text-white hover:shadow-lg hover:shadow-[#1E4D2B]/30 p-6 border border-[#FF3333]/20 hover:border-[#1E4D2B]"
    ...
    >
      ...
      <div className="relative h-[1px] my-2 overflow-hidden">
        <div 
          className="absolute inset-0 mx-auto bg-gradient-to-r from-transparent via-[#1E4D2B] to-transparent"
          style={{
            ...
            <div className="text-gray-300 text-sm mb-4 w-full">
              <div className="flex items-start gap-4">
                ...
                <div className="relative w-24 h-24 rounded-md overflow-hidden flex-shrink-0">
                  <img
                    src={post.thumbnail || '/jwt.webp'}
                    alt="thumbnail"
                    className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                    onError={(e) => {
                      // 이미지 로드 실패 시 기본 이미지로 대체
                      (e.target as HTMLImageElement).src = '/jwt-logo.webp';
                    }}
                  />
                </div>
      ...
      <div className="flex items-center justify-between text-xs text-gray-400">
        ...
        <div className="flex items-center gap-2">
          {post.tags.map(tag => (
            <span 
              key={tag}
              className="px-2 py-1 bg-[#1E4D2B]/10 text-gray-300 rounded-full text-xs border border-[#FF3333]/20"
            >
              {tag}
            </span>
          ))}
