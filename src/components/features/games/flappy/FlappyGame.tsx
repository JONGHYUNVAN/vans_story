...
function rGate(ctx: CanvasRenderingContext2D, o: Obj, ts: number) {
  const topH = o.cy - GATE_GAP / 2, botY = o.cy + GATE_GAP / 2;
  const botH = H - botY - GH;
  const p = 0.55 + 0.45 * Math.sin(ts * 0.004 + o.x * 0.018);
  ctx.fillStyle = 'rgba(9,7,28,0.94)';
  ctx.fillRect(o.x, 0, GATE_W, topH); ctx.fillRect(o.x, botY, GATE_W, botH);
  ctx.strokeStyle = 'rgba(109,40,217,0.4)'; ctx.lineWidth = 1;
  ctx.strokeRect(o.x+.5, .5, GATE_W-1, topH-.5);
  ctx.strokeRect(o.x+.5, botY+.5, GATE_W-1, botH-.5);
  ctx.save(); ctx.strokeStyle = 'rgba(109,40,217,0.12)'; ctx.lineWidth = 0.6;
  for (let y = 14; y < topH-6; y += 14) { ctx.beginPath(); ctx.moveTo(o.x+6, y); ctx.lineTo(o.x+GATE_W-6, y); ctx.stroke(); }
  for (let y = botY+8; y < botY+botH-6; y += 14) { ctx.beginPath(); ctx.moveTo(o.x+6, y); ctx.lineTo(o.x+GATE_W-6, y); ctx.stroke(); }
  ctx.restore();
  ctx.shadowBlur = 16*p; ctx.shadowColor = '#e879f9';
  ctx.fillStyle = `rgba(232,121,249,${0.88*p})`;
  ctx.fillRect(o.x, topH-3, GATE_W, 3); ctx.fillRect(o.x, botY, GATE_W, 3);
  ctx.shadowBlur = 0;
  const nx = o.x + GATE_W/2;
  ctx.shadowBlur = 12*p; ctx.shadowColor = '#e879f9'; ctx.fillStyle = '#f0abfc';
  ctx.beginPath(); ctx.arc(nx, topH-1.5, 4.5, 0, Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.arc(nx, botY+1.5, 4.5, 0, Math.PI*2); ctx.fill();
  ctx.shadowBlur = 0;
}
  ...
  for (const o of objs) {
    if (o.kind === 'gate') {
      if (BX+13 > o.x+2 && BX-13 < o.x+GATE_W-2)
        if (by-10 < o.cy-GATE_GAP/2 || by+10 > o.cy+GATE_GAP/2) return true;
    } else {
...
export default function FlappyGame({ onGameEnd, onScoreChange, onAction, onCombo }: GameComponentProps) {
  ...
  const spawnObj = useCallback((): Obj => {
    const mc = Math.min(0.55, 0.1 + scoreRef.current * 0.04);
    if (Math.random() < mc) {
      const spd = (0.9 + Math.random() * 1.7) * (1 + scoreRef.current * 0.025);
      return {
        x: W + 10, kind: 'mine',
        cy: 65 + Math.random() * (H - GH - 130),
  ...
  const draw = useCallback((ts: number) => {
    ...
    if (playing) {
      ...
      objsRef.current = objsRef.current.filter(o => o.x + (o.kind === 'gate' ? GATE_W : o.r + 12) > -10);
      ...
      // 점수
      for (const o of objsRef.current) {
        const edge = o.kind === 'gate' ? o.x + GATE_W : o.x - o.r;
      ...
      if (hitTest(byRef.current, objsRef.current)) {
        ...
        objsRef.current.forEach(o => o.kind === 'gate' ? rGate(ctx, o, ts) : rMine(ctx, o, ts));
    ...
    <div className="flex flex-col items-center gap-3 p-4">
      <div className="relative select-none" style={shakeStyle}>
        <canvas
          ...
          style={{ border: '1px solid rgba(109,40,217,0.28)', background: '#03030b' }}
          onMouseDown={() => {
      ...
      <p className="text-zinc-600 text-xs font-mono">hold space / click / TAP · fly up</p>
      ...
      <button
        ...
        className="w-full max-w-[320px] py-6 bg-violet-800/60 hover:bg-violet-700/60 active:bg-violet-600/60 border border-violet-500/40 rounded-2xl text-violet-200 text-lg font-mono font-bold tracking-[0.3em] select-none touch-none mt-2"
      >
        TAP
      </button>
    </div>
